...
t_client *addClient(t_server *server, int fd, struct sockaddr_in *client_addr) {
    t_client *new_client;
    ...
    new_client->fd = fd;
    new_client->msg = NULL;
    new_client->addr = *client_addr; // Store client address
...
void    sendNotification(t_server *server, int author, char *str) {
    t_client    *client;
...
void registerClient(t_server *server, int fd, struct sockaddr_in *client_addr) {
    t_client *new_client;
    char buf_write[256];

    new_client = addClient(server, fd, client_addr);
...
void acceptRegistration(t_server *server) {
    socklen_t addr_len;
    int connfd;
    struct sockaddr_in client_addr; // Temporary client address

    addr_len = sizeof(client_addr);
    connfd = accept(server->sockfd, (struct sockaddr *)&client_addr, &addr_len);
    if (connfd < 0) fatalError(server);
    registerClient(server, connfd, &client_addr);
}
